...
async function ensureStateSheet(service: sheets_v4.Sheets, id: string) {
...
export async function readHouseholdState(): Promise<StateEnvelope | null> {
  const service = sheetsService();
  const id = spreadsheetId();
  await ensureStateSheet(service, id);
  ...
  if (parsed.version !== 1 || typeof parsed.revision !== "string" || typeof parsed.updatedAt !== "string") {
    throw new Error("Stored state has an unsupported format.");
  }
  return parsed;
}

export async function writeHouseholdState(state: HouseholdState, expectedRevision: string | null) {
  const service = sheetsService();
  const id = spreadsheetId();
  const current = await readHouseholdState();
...
export async function deleteHouseholdState(expectedRevision: string | null) {
  const service = sheetsService();
  const id = spreadsheetId();
  const current = await readHouseholdState();
  if ((current?.revision ?? null) !== expectedRevision) {
